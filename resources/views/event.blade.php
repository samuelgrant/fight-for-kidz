								...
								<div class="bout-card">
									...
									<div class="contender-card contender-card-red">
										<div class="contender-card-inner">
											<img src="{{file_exists(public_path('/storage/images/contenders/' . $bout->red_contender->id . '.jpg')) ? '/storage/images/contenders/' . $bout->red_contender->id . '.jpg' : '/storage/images/contenders/0.png'}}" class="mx-auto contender-img">
											<div class="contender-name">
												<h5>{{$bout->red_contender->first_name}}</h5>
												<div class="nickname-wrapper">
													<h4 class="nickname-cell">{{$bout->red_contender->nickname}}</h4>
												</div>
												...
												@if(App\Contender::find($bout->red_contender_id)->sponsor_id)
													@if(App\Sponsor::find(App\Contender::find($bout->red_contender_id)->sponsor_id)->url)              						                
														<a href="{{App\Sponsor::find(App\Contender::find($bout->red_contender_id)->sponsor_id)->url}}" target="blank" class="contender-sponsor-logo-wrapper">
															<img id="red-sponsor" src="{{file_exists(public_path('/storage/images/sponsors/' . App\Contender::find($bout->red_contender_id)->sponsor_id . '.png')) ? '/storage/images/sponsors/' . App\Contender::find($bout->red_contender_id)->sponsor_id . '.png' : '/storage/images/sponsors/0.png'}}" class="mx-auto contender-sponsor-logo" title="{{App\Sponsor::find(App\Contender::find($bout->red_contender_id)->sponsor_id)->company_name}}">
														</a>
													@else
											...
											<div class="bout-btn bout-btn-red bio-view-button" data-toggle="modal" data-target="#bio-modal" data-contenderId="{{$bout->red_contender->id}}">
												View Bio
											</div>
											<div class="bout-btn bout-btn-red {{$bout->red_contender->donate_url ? '' : 'invisible'}}" onclick="window.open('{{$bout->red_contender->donate_url}}', '_blank')">
												Donate
											</div>
										</div>
									</div>
									...
									<div class="contender-card contender-card-blue">
										<div class="contender-card-inner">
											<img src="{{file_exists(public_path('/storage/images/contenders/' . $bout->blue_contender->id . '.jpg')) ? '/storage/images/contenders/' . $bout->blue_contender->id . '.jpg' : '/storage/images/contenders/0.png'}}" class="mx-auto contender-img">
											<div class="contender-name">
												<h5>{{$bout->blue_contender->first_name}}</h5>
											<div class="nickname-wrapper">
												<h4 class="nickname-cell">{{$bout->blue_contender->nickname}}</h4>
											</div>
											...
											@if(App\Contender::find($bout->blue_contender_id)->sponsor_id)   
												@if(App\Sponsor::find(App\Contender::find($bout->blue_contender_id)->sponsor_id)->url)
													<a href="{{App\Sponsor::find(App\Contender::find($bout->blue_contender_id)->sponsor_id)->url}}" target="blank" class="contender-sponsor-logo-wrapper">
														<img id="blue-sponsor" src="{{file_exists(public_path('/storage/images/sponsors/' . App\Contender::find($bout->blue_contender_id)->sponsor_id . '.png')) ? '/storage/images/sponsors/' . App\Contender::find($bout->blue_contender_id)->sponsor_id . '.png' : '/storage/images/sponsors/0.png'}}" class="mx-auto contender-sponsor-logo" title="{{App\Sponsor::find(App\Contender::find($bout->blue_contender_id)->sponsor_id)->company_name}}">
													</a>
												@else
													<div class="contender-sponsor-logo-wrapper">
														<img id="blue-sponsor" src="{{file_exists(public_path('/storage/images/sponsors/' . App\Contender::find($bout->blue_contender_id)->sponsor_id . '.png')) ? '/storage/images/sponsors/' . App\Contender::find($bout->blue_contender_id)->sponsor_id . '.png' : '/storage/images/sponsors/0.png'}}" class="mx-auto contender-sponsor-logo" title="{{App\Sponsor::find(App\Contender::find($bout->blue_contender_id)->sponsor_id)->company_name}}">
										...
										<div class="bout-btn bout-btn-blue bio-view-button" data-toggle="modal" data-target="#bio-modal" data-contenderId="{{$bout->blue_contender->id}}">
											View Bio
										</div>
										<div class="bout-btn bout-btn-blue {{$bout->blue_contender->donate_url ? '' : 'invisible'}}" onclick="window.open('{{$bout->blue_contender->donate_url}}', '_blank')">
											Donate
										</div>
									</div>
								</div>
							...
							<div class="col-lg-6 bout-column">
								...
								<div class="bout-card">
									...
									<div class="bout-footer mx-auto mt-3 w-100">
										<div class="bout-btn-fight"  data-toggle="modal" data-target="#fight-video-modal" data-bout-id="{{$bout->id}}">
											Watch the Fight!
										</div>
									</div>
									@endif
										...
										<table id="contenderTable" class="table table-sm text-center">
											<tbody>
												<tr>
													<td>
														Age: 
														<span id="contenderAge"></span></td>
												</tr>
												<tr>
													<td>
														Weight (kg): 
														<span id="contenderWeight"></span>
													</td>
												</tr>
												<tr>
													<td>
														Height (cm):
														 <span id="contenderHeight"></span>
													</td>
												</tr>
												<tr>
													<td>
														Reach (cm): 
														<span id="contenderReach"></span>
													</td>
												</tr>
											</tbody>
										</table>
